...
for /f "delims=. tokens=1-3" %%v in ("%JAVA_VERSION%") do (
    ...
    set PERM_SIZE=
    if "%MINOR%" LSS "8" (
      set PERM_SIZE=-XX:PermSize=256M -XX:MaxPermSize=1024M
    )
...
echo "%_JAVACMD%" %MEM_OPTS% %ACTIVATOR_OPTS% %SBT_OPTS% %_JAVA_OPTS% "-Dactivator.home=%JAVA_FRIENDLY_HOME%" -jar "%ACTIVATOR_HOME%\%ACTIVATOR_LAUNCH_JAR%" %CMDS%
